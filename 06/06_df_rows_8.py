# zaimportowanie biblioteki
import pandas as pd

# wyświetlenie większej ilości kolumn
desired_width = 350
pd.set_option('display.width', desired_width)
pd.set_option('display.max_columns', 15)

# wczytanie danych / należy zwrócić uwagę na sep i decimal xxx
df_first = pd.read_csv("Dane_01.csv", sep=";", decimal=',')
df_second = pd.read_csv("Dane_02.csv", sep=";")

# łączenie danych metodą merge xxx
df_animals = pd.merge(df_first, df_second, how='outer')

# cięcie rekordów
print(df_animals[0:4])
print("\n")
print(df_animals[:4])
print("\n")
print(df_animals[:])
print("\n")
print(df_animals[::-1])
print("\n")
print(df_animals[::2])
print("\n")
print(df_animals[::2][:6])
print("\n")
print(df_animals.iloc[::2, :].iloc[::-1][0:10])
print("\n")
print(df_animals[2:])
print("\n")
print(df_animals.iloc[1::5, :])
print("\n")
print(df_animals[df_animals.index % 3 != 0])
print("\n")
print(df_animals[0:10][['Gatunek']])
