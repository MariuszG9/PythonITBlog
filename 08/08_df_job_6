...
def create_columns():
    ...
    merged_df_ws.to_csv('ramka_wynikowa.csv', sep=';', index=False)
...
def use_them_all():
    print("Wybrałeś ukrytą operację przetwarzania całościowego UTA")
    data_frame_operation(f"----Początek operacji z ukrytą funkcją UTA----")
    load_csv()
    delete_duplicates()
    delete_nulls()
    remove_column()
    ...
    remove_column()
    load_frame()
    data_frame_operation(f"----Koniec operacji z ukrytą funkcją UTA----")
