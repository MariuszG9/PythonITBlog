...
def data_analysis_ppu():
    ...
    # Etykiety danych
    #for i, v in enumerate(merged_df_database['value'] / 1000):
    #    ax1.text(i + 0.5, 0.1, f"{v:.0f}", ha='center', fontsize=10, color="grey")

    for i, v in enumerate(merged_df_database['price_per_unit']):
        ax2.text(i - 0.0, v + 0.00, f"{v:.2f}", color=price_color)
