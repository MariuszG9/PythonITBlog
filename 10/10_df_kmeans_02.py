...
def kmeans_analyze2():
    ...
    merged_df_database['customer'] = merged_df_database['customer'].apply(lambda x: customer_map[x[0]])

    # wyodrębnienie kolumn "price_per_unit" i "customer"
    x = merged_df_database[['price_per_unit', 'customer']].values
    ...
    # dodanie informacji o klastrach do ramki danych
    merged_df_database['cluster'] = labels

    # wyświetlenie wykresu punktowego
    fig, ax = plt.subplots()
    ax.scatter(x_norm[:, 1], merged_df_database['price_per_unit'], c=labels, cmap='viridis')
    ...
    for label in unique_labels:
        ...
        ax.annotate(f'Customer {chr(65+label)}', xy=(x_norm[index, 1], merged_df_database['price_per_unit'][index]),
    ...
    merged_df_database['cluster_name'] = merged_df_database['cluster'].apply(lambda x: cluster_names[x])
    print(merged_df_database.groupby('cluster_name')[['price_per_unit', 'customer']].describe())
