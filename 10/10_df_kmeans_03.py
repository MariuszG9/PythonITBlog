...
def kmeans_analyze3():
    ...
    customer_map = {'AA': 0, 'AB': 1, 'AC': 2, 'AD': 3, 'AF': 4, 'AG': 4, 'AH': 4, 'AM': 5, 'AN': 5, 'BA': 9, 'BB': 10, 'CA': 11}
    ...
    fig, ax = plt.subplots()
    ax.scatter(cluster_df['customerx'], x_original[:, 0], c=labels, cmap='viridis')
    ax.set_title('Klasteryzacja na podstawie price_per_unit i customer')
    ax.set_xlabel('customer')
    ax.set_ylabel('price_per_unit')
    ...
    for i in range(len(x_original)):
        ax.annotate(f"{cluster_df['new_customer'][i]}[{cluster_df['market'][i]}]",
                    xy=(cluster_df['customerx'][i], x_original[i, 0]),
                    xytext=(5, 0),
    ...
    ax.set_xlim([cluster_df['customerx'].min(), cluster_df['customerx'].max()])
    ax.set_ylim([cluster_df['price_per_unit'].min(), cluster_df['price_per_unit'].max()])
    ax.autoscale()
