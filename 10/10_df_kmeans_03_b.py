...
def kmeans_analyze():
    cluster_df = pd.read_csv('ramka_wynikowa_eur.csv', sep=";", encoding='utf-8')

    # wyodrębnienie kolumn "price_per_unit" i "quantity"
    x = cluster_df[['price_per_unit', 'quantity']].values
    ...
    print(cluster_df.groupby('cluster')[['price_per_unit', 'quantity']].describe())


def kmeans_analyze2():
    cluster_df = pd.read_csv('ramka_wynikowa_eur.csv', sep=";", encoding='utf-8')

    # mapowanie pierwszej litery w kolumnie customer na wartości liczbowe
    customer_map = {'A': 0, 'B': 1, 'C': 2}
    cluster_df['customer'] = cluster_df['customer'].apply(lambda x: customer_map[x[0]])

    # wyodrębnienie kolumn "price_per_unit" i "customer"
    x = cluster_df[['price_per_unit', 'customer']].values
    ...
    # wyświetlenie wykresu punktowego
    fig, ax = plt.subplots()
    ax.scatter(x_norm[:, 1], cluster_df['price_per_unit'], c=labels, cmap='viridis')
    ax.set_title('Klasteryzacja na podstawie price_per_unit i customer')
    ax.set_xlabel('customer')
    ax.set_ylabel('price_per_unit')

    # dodanie etykiet dla pierwszego punktu w każdym klastrze
    unique_labels = np.unique(labels)
    for label in unique_labels:
        index = np.where(labels == label)[0][0]
        ax.annotate(f'Customer {chr(65 + label)}', xy=(x_norm[index, 1], cluster_df['price_per_unit'][index]),
                    xytext=(5, 0), textcoords='offset points', fontsize=8, color='gray')

    plt.show()

    # wyświetlenie podsumowania klastrów
    cluster_names = {0: 'Customer A', 1: 'Customer B', 2: 'Customer C'}
    cluster_df['cluster_name'] = cluster_df['cluster'].apply(lambda x: cluster_names[x])
    print(cluster_df.groupby('cluster_name')[['price_per_unit', 'customer']].describe())

    wcss = []
    for i in range(1, 9):
        kemans = KMeans(i)
        kemans.fit(x_norm)
        wcss_iter = kemans.inertia_
        wcss.append(wcss_iter)
    # kod z użyciem LC: wcss = [(KMeans(i).fit(x_norm).inertia_) for i in range(1, 9)]
    print(wcss)

    numb_clusters = range(1, 9)
    plt.plot(numb_clusters, wcss, linewidth=2, color='#FFD966')
    plt.gca().add_patch(plt.Circle((2, wcss[1]), radius=0.15, edgecolor='#9F1166', fill=False, linewidth=2, zorder=2))
    plt.gca().add_patch(plt.Circle((3, wcss[2]), radius=0.15, edgecolor='#9F1166', fill=False, linewidth=2, zorder=2))
    plt.show()
...
def kmeans_analyze3():
    ...
    # mapowanie pierwszych dwóch liter w kolumnie customer na wartości liczbowe
    customer_map = {'AA': 0, 'AB': 1, 'AC': 1, 'AD': 1, 'AF': 1, 'AG': 1, 'AH': 1, 'AM': 1, 'AN': 1,
                    'BA': 9, 'BB': 10, 'CA': 11}
    cluster_df['customerx'] = cluster_df['customer'].apply(lambda x: customer_map[x[:2]])

    # wyodrębnienie kolumn "price_per_unit" i "customer"
    x = cluster_df[['price_per_unit', 'customerx']].values

    # normalizacja danych
    scaler = MinMaxScaler()
    x_norm = scaler.fit_transform(x)

    wcss = [KMeans(i).fit(x_norm).inertia_ for i in range(1, 9)]
    numb_clusters = range(1, 9)
    plt.plot(numb_clusters, wcss, linewidth=2, color='#FFD966')
    plt.show()

    # wykonanie analizy klastrowania za pomocą KMeans
    number_of_clusters = int(input("Podaj na ile klastrów chcesz podzielić dane: "))
    ...
    # przetransformowanie wartości osi na oryginalne wartości
...
def data_filtering(df_dict):
    ...
    if dtype.startswith('float') or dtype.startswith('int'):
        ...
        while not is_valid:
            choice = input(
                f"Kolumna {selected_column_name} zawiera liczby. Wybierz:\n 1 - konkretną wartość\n 2 - zakres wartości (od - do)\n")
            if choice == '1':
                filter_value = float(
                    input(f"Podaj wartość, po której chcesz filtrować kolumnę {selected_column_name}: "))
                is_valid = True
            elif choice == '2':
                min_value = float(input(
                    f"Podaj minimalną wartość z zakresu, po którym chcesz filtrować kolumnę {selected_column_name}: "))
                max_value = float(input(
                    f"Podaj maksymalną wartość z zakresu, po którym chcesz filtrować kolumnę {selected_column_name}: "))
    ...
    if isinstance(filter_value, tuple):
        filtered_df = selected_df[(selected_df[selected_column_name] >= filter_value[0]) & (
                    selected_df[selected_column_name] <= filter_value[1])]
    else:
